...
```lua
-- pullhelper.lua
-- Registers decision functions LavishScript can call via ${ISXLUA.Call[...]}.
-- Run with:  lua pullhelper     (stop with:  endlua pullhelper)
...
local function ensureISXEQ2()
    ...
    return waituntil(function() local e = ISXEQ2; return e ~= nil and e.IsReady end, 30)
end
...
-- Return true only when it is safe to pull: full-ish health/power and no encounter.
IS.Register("IsSafeToPull", function()
    return Me.Health >= 90 and Me.Power >= 50 and not Me.InCombat
end)

-- Pick the nearest NPC within a caller-supplied range; return its name (or nothing).
-- Args arrive as strings, so convert range with tonumber.
IS.Register("NearestNPCName", function(range)
    local maxRange = tonumber(range) or 30
    local actor = Actor("query", "Type =- \"NPC\" && Distance <= " .. maxRange, "sort", "Distance")
    if Exists(actor) then
        return actor.Name          -- a string -> LavishScript sees a string
    end
    -- returning nothing -> ${ISXLUA.Call[...]} is NULL
end)

echo("pullhelper ready -- LavishScript can now call IsSafeToPull / NearestNPCName.")

-- Stay alive so the registrations persist. They auto-clear on endlua pullhelper.
while true do
    wait(5)
end
```

From a LavishScript `.iss` bot (or typed at the console), call the registered
functions and branch on the results:

```
if ${ISXLUA.Call[IsSafeToPull]}
{
    variable string mob = ${ISXLUA.Call[NearestNPCName, 25]}
    if ${mob.Length}
    {
        echo Pulling ${mob}
...
Or interactively, to test without a bot:

```
luacall IsSafeToPull
    ISXLUA: IsSafeToPull -> TRUE
luacall NearestNPCName 25
    ISXLUA: NearestNPCName -> a decrepit servant
```

Why it is shaped this way:

- **The Lua script owns the decision logic** and can use the full object model
  (`Me.Health`, `Actor("query", ...)`), while the LavishScript bot just asks for a
  yes/no or a name -- a clean way to migrate bot logic to Lua incrementally.
- **The registered functions never `wait()`.** They run atomically when called and
  return immediately; the `while true do wait(5) end` loop exists only to keep the
  script (and therefore its registrations) alive.
