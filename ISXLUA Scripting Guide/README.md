...
| [`02_The_IS_Bridge.md`](02_The_IS_Bridge.md) | The `IS` global -- `IS.Execute` / `IS.Parse` (escape hatches into LavishScript), the typed LavishScript interop layer (`IS.GetVar` / `IS.SetVar` read/write LavishScript variables, `IS.CallAtom` calls a LavishScript atom), `print` / `echo`, persistent storage (`IS.SaveTable` / `IS.LoadTable` and the hierarchical `IS.Settings` config store), and the reverse bridge (call Lua from LavishScript). |
...
| [`04_Timing_And_Events.md`](04_Timing_And_Events.md) | `wait(seconds)` / `waitframe()` / `waituntil` / `waitforevent`, timers (`setTimeout` / `setInterval` / `clearTimer`), await-style async (`await` -- turn a callback into a linear call), the events layer (`IS.AttachEvent` / `IS.AttachEventTyped` / `IS.DetachEvent` / `IS.FireEvent` / `IS.EventSource`) with atomic handlers, sharing data between scripts (the `IS.Share` / `IS.Shared` value store and the `IS.Publish` / `IS.Subscribe` message bus), cross-session relay for multiboxing (`IS.Relay` runs a command, `IS.RelayEvent` fires an event, in your other game sessions -- received with `IS.AttachEvent`), and asynchronous HTTP (`IS.HttpGet` / `IS.HttpPost`, plus the inline `IS.HttpGetSync` / `IS.HttpPostSync` wrappers -- "with libisxgames" build only). |
...
| [`06_Bundled_Libraries.md`](06_Bundled_Libraries.md) | The `require`-able modules that ship inside ISXLUA (`cjson`, `lfs`, `lpeg`, `serpent`, `inspect`, `json`, `re`, `socket` networking, `ssl` / `ssl.https` TLS + HTTPS, `zlib` compression, `lsqlite3` database, `sha2` / `base64` / `crypto` hashing + encoding, `MessagePack` binary serialization, `luaunit` unit testing, `lgui2` / `lgui1` UIs, `middleclass` classes, the `Penlight` `pl.*` standard-library extensions, and the optional `isxlua` bridge/runtime convenience layer, and the `input` keyboard/mouse/bind automation module) and how to load your own loose modules. |
