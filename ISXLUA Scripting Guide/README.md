...
| [`04_Timing_And_Events.md`](04_Timing_And_Events.md) | `wait(seconds)` / `waitframe()` / `waituntil` / `waitforevent`, timers (`setTimeout` / `setInterval` / `clearTimer`), and the events layer (`IS.AttachEvent` / `IS.DetachEvent` / `IS.FireEvent` / `IS.EventSource`) with atomic handlers. |
